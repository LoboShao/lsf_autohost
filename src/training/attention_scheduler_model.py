...
class AttentionSchedulerPolicy(nn.Module):
    ...
    def __init__(self, obs_dim, action_dim, num_hosts=30, exploration_noise_decay=0.995, min_exploration_noise=0.01):
        ...
        self.job_encoder = nn.Sequential(
            nn.Linear(4, 32),  # cores, memory, duration, is_deferred
            # nn.LayerNorm(32),
            nn.GELU(),
        ...
        self.queue_encoder = nn.Sequential(
            nn.Linear(4, 64),  # batch_progress, queue_pressure, core_pressure, memory_pressure
            # nn.LayerNorm(64),
            nn.GELU(),
    ...
    def forward(self, obs, deterministic=False):
        ...
        num_hosts = (obs.shape[1] - 8) // 2  # Updated: 8 job features
        
        # Parse observation: [host_features[2*num_hosts], 8 job/queue features]
        host_features = obs[:, :num_hosts * 2].view(batch_size, num_hosts, 2)  # [batch, num_hosts, 2]
        
        # Job features (4 features) - intrinsic job properties
        job_features = torch.stack([
            obs[:, num_hosts * 2],      # job_cores_norm
            obs[:, num_hosts * 2 + 1],  # job_mem_norm  
            obs[:, num_hosts * 2 + 2],  # job_duration_norm
            obs[:, num_hosts * 2 + 3],  # is_deferred
        ], dim=1)  # [batch, 4]
        
        # Queue/Context features (4 features) - scheduling context
        queue_features = torch.stack([
            obs[:, num_hosts * 2 + 4],  # batch_progress
            obs[:, num_hosts * 2 + 5],  # queue_pressure
            obs[:, num_hosts * 2 + 6],  # core_pressure
            obs[:, num_hosts * 2 + 7],  # memory_pressure
        ], dim=1)  # [batch, 4]
